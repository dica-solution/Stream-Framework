...
from feeds.tasks import realtime_fanout
...
feed_add_processed = Signal(providing_args=["activities", "aggregated"])
feed_remove_processed = Signal(providing_args=["activities", "aggregated"])
...
def add_operation(feed, activities, trim=True, batch_interface=None):
    ...
    t = timer()
    msg_format = (
        "running %s.add_many operation for %s activities batch interface %s and trim %s"
    )
    logger.debug(msg_format, feed, len(activities), batch_interface, trim)
    activities_processed = feed.add_many(
        activities, batch_interface=batch_interface, trim=trim
    )
    feed_add_processed.send(
        sender=feed,
        activities=activities_processed,
        aggregated=hasattr(feed, "aggregator_class"),
    )
...
def remove_operation(feed, activities, trim=True, batch_interface=None):
    ...
    activities_processed = feed.remove_many(
        activities, trim=trim, batch_interface=batch_interface
    )
    feed_remove_processed.send(
        sender=feed,
        activities=activities_processed,
        aggregated=hasattr(feed, "aggregator_class"),
    )
    ...
    **Example** ::
    ...
    def batch_import(self, user_id, activities, fanout=True, chunk_size=500):
        ...
        **Example**::
        ...
        logger.info(
            "processing %s items in %s chunks of %s",
            len(activities),
            len(activity_chunks),
            chunk_size,
        )
        ...
        for index, activity_chunk in enumerate(activity_chunks):
            ...
            logger.info(
                "inserted chunk %s (length %s) into the user feed",
                index,
                len(activity_chunk),
            )
